...
-- ENUM  is a data type in MySQL used to define a column that can only hold one value from a predefined list
...
CREATE PROCEDURE transfer_funds10()
BEGIN
    ...
    -- Step 2: Attempt to deduct ₹2000 (will fail if balance < 2000)
     insert into accounts values ('e123','renu',3000);
    UPDATE accounts
    ...
    select rows_changed AS rows_affected;
...
CREATE PROCEDURE transfer_funds6()
BEGIN
    DECLARE rows_changed INT;
    ...
    -- Step 2: Attempt to deduct ₹2000 (will fail if balance < 2000)
    insert into accounts values ('e123','ritu',3000);
    UPDATE accounts
